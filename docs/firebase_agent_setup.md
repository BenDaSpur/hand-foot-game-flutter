...
Secrets are injected as environment variables (`CLOUD_AGENT_INJECTED_SECRET_NAMES`). They are **not** on disk until bootstrapped:
...
Node analytics scripts prefer **service account** auth (`FIREBASE_HAND_FOOT_SERVICE_ACCOUNT_B64` or `.firebase/hand-foot-service-account.json`). OAuth user tokens are a legacy fallback via `scripts/analytics_http_common.js`.
