...
void main() {
  group('Player', () {
    ...
    test('should deal cards to hand and foot', () {
      ...
      expect(player.currentHand, equals(handCards)); // Should use hand initially
    });
    ...
    test('should not pick up foot if hand is not empty', () {
      final handCards = [const PlayingCard(suit: Suit.hearts, rank: CardRank.ace)];
      final footCards = [const PlayingCard(suit: Suit.clubs, rank: CardRank.queen)];
    ...
    test('should add and remove cards from current hand', () {
      ...
      final notRemoved = player.removeCardFromHand(const PlayingCard(suit: Suit.clubs, rank: CardRank.queen));
    ...
    test('should calculate hand and meld values correctly', () {
      player.dealHand([
        ...
        const PlayingCard(suit: Suit.hearts, rank: CardRank.three), // -300 points (red 3)
      ]);
      
      // Add a meld manually for testing
      final meld = Meld(
        rank: CardRank.queen,
        cards: List.generate(7, (i) => const PlayingCard(suit: Suit.hearts, rank: CardRank.queen)),
    ...
    test('should sort hand by different criteria', () {
      ...
      expect(player.currentHand.first.rank, equals(CardRank.two)); // Lowest meld value
      expect(player.currentHand.last.rank, equals(CardRank.joker)); // Jokers go last
      
      // Reset and sort by suit
      player.dealHand(List.from(cards));
      player.sortHandBySuit();
      expect(player.currentHand.last.rank, equals(CardRank.joker)); // Jokers still last
      
      // Reset and sort by point value
      player.dealHand(List.from(cards));
      player.sortHandByValue();
      expect(player.currentHand.first.pointValue, equals(5)); // Four has lowest point value
      expect(player.currentHand.last.pointValue, equals(50)); // Joker has highest
    });
