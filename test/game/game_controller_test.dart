...
void main() {
  group('GameController', () {
    ...
    test('should initialize and deal cards', () {
      ...
      expect(gameController.gameState.currentPlayer.type, equals(PlayerType.human));
    });
    ...
    test('should handle discarding', () {
      ...
      final initialDiscardPileSize = gameController.gameState.discardPile.length;
      ...
      expect(success, isTrue);
      expect(gameController.gameState.discardPile.length, equals(initialDiscardPileSize + 1));
    ...
    test('should export and import game state', () {
      ...
      expect(newController.gameState.hasDrawnFromDeck, equals(gameController.gameState.hasDrawnFromDeck));
    });

    test('should handle seeded random generation consistently', () {
      const seed = 12345;
      
      final controller1 = GameController(players: [
        Player(id: '1', name: 'Player 1', type: PlayerType.human),
        Player(id: '2', name: 'Player 2', type: PlayerType.bot),
      ], seed: seed);
      
      final controller2 = GameController(players: [
        Player(id: '1', name: 'Player 1', type: PlayerType.human),
        Player(id: '2', name: 'Player 2', type: PlayerType.bot),
      ], seed: seed);
      ...
      expect(controller1.gameState.players[0].hand.length, 
             equals(controller2.gameState.players[0].hand.length));
      expect(controller1.gameState.players[0].foot.length, 
             equals(controller2.gameState.players[0].foot.length));
      
      // First cards should be the same
      expect(controller1.gameState.players[0].hand.first.rank,
             equals(controller2.gameState.players[0].hand.first.rank));
      expect(controller1.gameState.players[0].hand.first.suit,
    ...
    test('should handle bot turn processing', () {
      ...
      // Advance to bot turn
      gameController.gameState.nextPlayer();
      expect(gameController.gameState.currentPlayer.type, equals(PlayerType.bot));
    ...
    test('should maintain game state consistency', () {
      ...
      if (totalCards != expectedCards) {
        ...
        for (int i = 0; i < gameController.gameState.players.length; i++) {
          ...
          print('  Player $i: hand=${player.hand.length}, foot=${player.foot.length}, melds=${player.melds.fold(0, (sum, meld) => sum + meld.cards.length)}');
        }
      }
