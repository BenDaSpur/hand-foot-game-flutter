...
- **Known caveat — starting a game hangs in the DEBUG web build:** clicking `PLAY SOLO` (or `START GAME`) in `flutter run` (debug) leaves the app stuck on a loading spinner. `GameScreen.initState` → `_startFreshGame` synchronously sets the Riverpod game-controller provider (`lib/providers/game_providers.dart` `_setControllerWithEventSubscription`), which trips Riverpod 3.x's "modify a provider while the widget tree was building" assertion. Asserts run only in debug, so this is invisible in release/profile builds and in production (playhandfoot.com). To exercise real gameplay locally, run a release/profile build instead: `flutter build web` then serve `build/web` (e.g. `python3 -m http.server 8081 --directory build/web`). The home/settings screens and the full `flutter test`/`flutter analyze` suites are unaffected.
