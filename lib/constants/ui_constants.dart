/// UI constants for consistent layout and sizing across the app
class UIConstants {
  // Mobile game board zones
  static const double compactHeaderCollapsedHeight = 72.0;
  static const double compactHeaderExpandedHeight = 120.0;
  static const double bottomDockMinHeight = 180.0;
  static const double handLabelHeight = 28.0;

  // Mobile status bar (legacy — compact header replaces on phone)
  static const double statusBarExpandedHeight = 120.0;
  static const double statusBarCollapsedHeight = 60.0;
  static const double statusBarPadding = 12.0;
  static const double statusBarIconSize = 14.0;
  static const double statusBarCompactChipFontSize = 12.0;

  // Recent actions
  static const double recentActionsMaxHeight = 120.0;
  static const double recentActionsFontSize = 11.0;
  static const double recentActionsLineHeight = 1.2;
  static const double recentActionsVerticalPadding = 2.0;
  static const int recentActionsPreviewLength = 40;

  // Screen breakpoints
  static const double smallScreenBreakpoint = 600.0;

  // Player scores
  // Tall enough for name + score rows with 1.5px current-player borders.
  static const double playerScoresHeight = 52.0;
  static const double playerScoresIconSize = 12.0;
  static const double playerScoresNameFontSize = 11.0;
  static const double playerScoresScoreFontSize = 13.0;
  static const double playerScoresMeldFontSize = 9.0;
  static const double playerScoresHandFootFontSize = 9.0;

  // General spacing
  static const double defaultMargin = 8.0;
  static const double defaultPadding = 8.0;
  static const double smallSpacing = 4.0;
  static const double mediumSpacing = 8.0;
  static const double largeSpacing = 16.0;

  /// Horizontal inset for the hand scroller so playable/newly-drawn glow
  /// shadows are not clipped on the leftmost/rightmost cards.
  /// Must cover playableOuterGlowBlur + playableOuterGlowSpread (12 + 2).
  static const double handGlowPadding = 14.0;

  // Border radius
  static const double defaultBorderRadius = 8.0;
  static const double smallBorderRadius = 4.0;
  static const double mediumBorderRadius = 6.0;

  // Alpha values
  static const double semiTransparent = 0.3;
  static const double lightTransparent = 0.1;
  static const double mediumTransparent = 0.8;
}
