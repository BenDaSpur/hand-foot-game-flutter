...
class GameState {
  ...
  void _logAction(String message, {bool showCardDetails = true}) {
    ...
    final isPublicAction = message.contains('discard') || 
    ...
    final finalMessage = shouldShowDetails ? message : _sanitizeMessage(message);
    
    recentActions.add(GameAction(
  ...
  bool get canEndTurn => 
      turnPhase == TurnPhase.discard && hasMelded;
  ...
  bool unlockDiscard() {
    ...
    if (additionalDiscards.isNotEmpty) {
      ...
      final additionalNames = additionalDiscards.map((c) => c.displayName).join(', ');
      _logAction('took ${additionalDiscards.length} more cards from discard pile: $additionalNames');
    }
  ...
  bool playMeld(List<PlayingCard> cards) {
    ...
    if (currentPlayer.createMeld(cards, playDownRequirement: playDownRequirement)) {
      hasMelded = true;
  ...
  bool immediateUnlock(int playerIndex) {
    if (playerIndex == currentPlayerIndex || playerIndex >= players.length) return false;
    if (discardPile.isEmpty) return false;
  ...
  void endRound() {
    ...
    final playerWhoWentOut = playersWhoCanGoOut.isEmpty ? null : playersWhoCanGoOut.first;
